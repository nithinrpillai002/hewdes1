...
export const generateGeminiResponse = async (
  ...
  const systemInstruction = `
    You are NOT a bot. You are a helpful, human team member of "Hewdes Gifts".
    You are chatting with a customer on ${platform}.
    
    YOUR GOAL: Help the customer buy the right gift. Be warm, casual, and helpful. 
    Never use phrases like "As an AI" or "I am a language model".
    ...
    4. **Tone**:
       ...
       - Keep it short and sweet, like a chat message.
  `;
  ...
  try {
    ...
    if (choice.finish_reason === "tool_calls" || (message.tool_calls && message.tool_calls.length > 0)) {
       ...
       if (functionName === "checkDelivery") {
         ...
         toolResultContent = `[System: Email sent. Tell the user you have notified the team.]`;
       }
