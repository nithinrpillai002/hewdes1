...
const CRM: React.FC<CRMProps> = ({ products, rules, setRules, kieApiKey }) => {
  // Main Platform Tab
  ...
  const [showGuide, setShowGuide] = useState(true); // Default open to help user
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (!kieApiKey) {
        const errorMsg: Message = {
            ...
            role: 'system',
            content: "⚠️ System Alert: Please configure the Kie API Key in the 'Settings' page to start the chat.",
            timestamp: Date.now()
        };
        ...
        <div className="space-y-4 text-sm text-slate-600 dark:text-slate-300">
            ...
            <div className="flex gap-3">
                ...
                <div>
                    ...
                    <p className="mt-1 text-xs text-red-500 font-bold bg-red-50 dark:bg-red-900/20 p-2 rounded">
                        IMPORTANT: Facebook requires a PUBLIC HTTPS URL. 
                        If you see 'localhost' below, you must deploy this app (e.g. to Cloud Run) or use ngrok.
                    </p>
                </div>
            </div>

            <div className="flex gap-3">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-100 dark:bg-indigo-900 text-indigo-700 dark:text-indigo-300 flex items-center justify-center font-bold text-xs">4</span>
                <div>
                    <p className="font-semibold text-slate-800 dark:text-white">Subscribe to Fields</p>
                    <p>After verifying, click "Manage" on Webhook fields.</p>
                    <p>Subscribe to: <strong>messages</strong> (WhatsApp) or <strong>messages</strong> (Instagram).</p>
                </div>
            </div>
        </div>
    </div>
                ...
                {isLocalhost && (
                    <div className="bg-red-50 dark:bg-red-900/20 p-3 rounded-lg border border-red-100 dark:border-red-800 flex items-start space-x-2">
                         ...
                         <div className="text-xs text-red-800 dark:text-red-300">
                            ...
                            <div className="mt-2 text-[10px] bg-white dark:bg-slate-800 p-2 rounded border border-red-200 dark:border-red-800">
                                <strong>Why?</strong> Facebook servers cannot reach your computer.
                                <br />
                                <strong>Fix:</strong> Deploy this code to a public server (Google Cloud Run, Vercel, Heroku) to get a public https://... URL.
                            </div>
                        </div>
                    </div>
                )}
      ...
      <div className="w-1/2 flex flex-col bg-slate-100 dark:bg-slate-950 border-l border-gray-200 dark:border-slate-800 relative">
        ...
        <div className="p-4 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 transition-colors">
            ...
            <p className="text-center text-[10px] text-slate-400 mt-2">
                Simulating Gemini 3 Pro environment via Kie API.
            </p>
        </div>
      </div>
    </div>
